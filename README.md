...
| Light on memory | Background tabs don't hold RAM forever. A tab left idle for a minute is **frozen** (its scripts and timers pause); left ten minutes, it is **discarded** — its render process is freed and the page reloads the moment you return to it. Tabs playing audio, downloading, or otherwise busy are left running (Vodou defers to the engine's own judgement), and **pinned tabs are never touched**. Switching to a tab always wakes it instantly |
