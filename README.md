...
| Drag links in | Drag a **link**, or the **site icon** next to another browser's address bar (Chrome, Edge, Firefox), and drop it on Vodou's tab strip — each opens in a new tab. Files from the file manager open too, and dropped text you didn't mean as a URL becomes a search. Dropped links pass the same deceptive-site check as anything you type, and `javascript:` payloads are refused. (Dragging a whole *tab* out of another browser can't work — that's a window-tear, not a link — so grab the address-bar icon instead) |
