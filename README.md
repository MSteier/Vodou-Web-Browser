...
| Location & region emulation | ☰ menu → Settings → Privacy & security → **Location & region…** — pick a region (New York, London, Tokyo, Sydney, Paris, Berlin, Toronto, São Paulo) and sites see that region's **browser language and locale**, applied **natively** (no script injection, no debug port): `navigator.language` / `navigator.languages` / `Accept-Language` change live, and `Intl` date/number/currency formatting after a restart (via Chromium's `--lang`). One coherent **Location Profile** drives it all, so nothing contradictory leaks. **Honest scope:** this emulates *language & locale only* — **not** your timezone, geolocation, or IP (those are shown for reference and left unchanged); a built-in Diagnostics view spells out exactly what is and isn't emulated |
