...
| Block Webcam | On by default (☰ menu → Settings → Block Webcam). Denies any page request for your camera at the engine level, so a site's `getUserMedia` fails without ever reaching the hardware — no JS workaround. Turn it off to be asked (Allow / Block) per site instead; takes effect on the next request, no reload needed. Microphone, screen capture, notifications, clipboard and font access stay denied by default regardless |
